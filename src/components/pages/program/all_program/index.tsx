...
interface AllProgramProps {
  data?: {
    listProgramAll?: Array<{
      ...
      listLogoProgramAll?: Array<{ logo?: { node?: { sourceUrl?: string } } }>;
    }>;
  };
}

export default function AllProgramSection({ data }: AllProgramProps) {
  const { normalizePath } = useStoreContext();
  const programs = data?.listProgramAll || [];
  return (
    <section className="sc-all-program">
      <div className="container-fluid container-2560">
        <div className="row">
          {programs.map((program, index) => {
            // Logic hiệu ứng AOS luân phiên (chẵn/lẻ) để giữ trải nghiệm cũ
            const isEven = index % 2 === 0;
            const aosImage = isEven ? "fade-right" : "fade-left";
            const aosInfo = isEven ? "fade-left" : "fade-right";

            return (
              <div className="main-program" key={index}>
                {/* ẢNH CHÍNH (Ảnh minh họa) */}
                <div className="image" data-aos={aosImage} data-aos-delay="500">
                  <img 
                ...
                {/* INFO BOX */}
                <div
                  className={`info-box ${program.backgroundImagePosition?.toLowerCase().replace(" ", "-") || "top-left"}`}
                  style={{
                    background: program.backgroundColor || "#003e58",
                    // Nếu bạn muốn dùng backGroundImage làm ảnh nền cho box:
                    backgroundImage: program.backGroundImage?.node?.sourceUrl 
                      ? `url(${program.backGroundImage.node.sourceUrl})` 
                      : undefined
                  }}
                  data-aos={aosInfo}
                >
                  ...
                  <div className="desc">{program.decs}</div>
                  ...
                  <div className="learn-more">
                    ...
                    <div className="logo">
                      {program.listLogoProgramAll?.map((l, i) => (
                        <img key={i} src={l.logo?.node?.sourceUrl} alt="Logo" />
                      ))}
                    </div>
                  </div>
                </div>
