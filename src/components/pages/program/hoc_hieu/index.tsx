...
export default function HocHieu({ data }: HocHieuProps) {
  const items = data?.listHocHieu || [];
  // Mặc định chọn item đầu tiên nếu có dữ liệu
  const [activeMenu, setActiveMenu] = useState(0);

  if (!items.length) return null;

  const activeContent = items[activeMenu];
  ...
  return (
    <section className="sc-hoc-hieu">
      <div className="inner-container">
        ...
        {data?.description && (
          <div className="description fw-300" data-aos="fade-up">
            {data.description}
          </div>
        )}
          ...
          <div className="menu">
            {items.map((item, index) => (
              <a
                key={index}
                href="#!"
                className={`menu-item ${activeMenu === index ? "is-hover" : ""}`}
                onMouseEnter={() => setActiveMenu(index)}
                onClick={(e) => e.preventDefault()}
              >
                <img src={item.image?.node?.sourceUrl} alt={item.title || "Menu"} />
              </a>
            ))}
          </div>
        </div>
        ...
        <div className="content" data-aos="fade-up">
          {activeContent && (
            <div className="menu-content show">
              <h4 className="menu-title">
                {/* Lấy ký tự đầu tiên làm 'letter' */}
                <span>{activeContent.title?.charAt(0)}</span> - {activeContent.title}
              </h4>

              <div 
                className="menu-desc fw-300" 
                dangerouslySetInnerHTML={{ __html: activeContent.contentDecs || "" }} 
              />
            </div>
          )}
        </div>
